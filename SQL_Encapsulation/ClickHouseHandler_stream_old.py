...
class ClickHouseHandler:
    def __init__(self, host=['localhost','localhost'], port=[9000,9000], user=['default','default'], password=['',''], database=['default','default'],prefix=None):
        ...
        self.read_client = Client(host=host[0], port=port[0], user=user[0], password=password[0],database=database[0],connect_timeout=6000)
        self.wd_client=Client(host=host[1], port=port[1], user=user[1], password=password[1],database=database[1],connect_timeout=6000)
    ...
    def _insert_into_select(self, source_sql: str, target_table: str, condit: dict):
        ...
        try:
            ...
            self.logger.error(f"执行_insert_into_select出错: {type(e).__name__}: {str(e)}，报错SQL：{sql}")
