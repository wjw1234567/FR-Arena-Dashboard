    ...
    select AA.profile_id
     ...
     (
        SELECT profile_id
             ...
             ,region_name
             , capture_time
             , capture_time - lagInFrame(capture_time, 1, capture_time) OVER (ORDER BY capture_time) timediff
             ,multiIf(
                   capture_time - lagInFrame(capture_time, 1, capture_time) OVER (ORDER BY capture_time) > 2
            ...
            WHERE toDate(capture_time) = %(date)s and region_name like  'CASINO%%'
