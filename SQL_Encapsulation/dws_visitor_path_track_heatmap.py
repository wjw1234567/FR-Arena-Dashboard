...
class TrackHeatmap:
    ...
    def process_one_person(df, target_bins):
        ...
        # 按小时进行分组
        # df_result['z0_time']=df_result['z0_time'].dt.floor("10min")
        df_result['z0_time'] = df_result['z0_time'].dt.floor("h")
        # 获取到根据时间差的绝对值
        df_result["abs_diff_min"] = df_result["diff_min"].abs()
        # 需要进行分组的字段
        group_keys = ["date", "date_hour", "profile_id", "region_name", "z0_time", "off_bin", "member_tier",
                      "age_range", "gender"]
        # 获取到距离基准地区的off_bin的最小时间的region
        min_diff_idx = df_result.groupby(group_keys

        )["abs_diff_min"].idxmin()
        df_result_group = df_result.loc[min_diff_idx]
        final_fields = ["date", "date_hour", "profile_id", "region_name", "z0_time", "off_bin", "area_at_off",
                        "member_tier", "age_range", "gender","batch_time"]

        if not results:
            return None

        return df_result_group[final_fields].reset_index(drop=True)
    ...
    def process_main(self,target_table,date_list):
        ...
        for date in date_list:
            try:
                ...
                # 使用多线程处理每个用户
                dfs = Parallel(n_jobs=-1)(delayed(self.process_one_person) (g, target_bins) for g in grouped)

                self.logger.log(f"{target_table}  {date} 已删除 {len(df_day)} 行")
                ...
                if dfs:
                    ...
                    for i in range(0,total_rows,batch_size):
                        ...
                        self.logger.log(
                            f"{target_table} {date} 第 {i // batch_size + 1} 批写入 {len(batch_df)} 行，累计 {min(i + batch_size, total_rows)}/{total_rows} 行"
                        )

                    # print(f"{date} 总条数写入 {len(big_df)} 行")

                    self.logger.log(f"{target_table} {date} 总条数写入 {len(big_df)} 行")


            except Exception as e:

                self.logger.error(f"{target_table} {date} 执行出错: {type(e).__name__}: {str(e)}")
